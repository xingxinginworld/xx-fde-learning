...
def retrieve(query, top_k=config.TOP_K, coarse_k=config.COARSE_K, threshold=config.SIM_THRESHOLD, role=None):
    ...
    参数：
      ...
      role      ：请求方角色（可选）。若分块带 'min_role' 权限元数据，
                 则过滤掉角色等级不足的块，防止越权召回（政企权限分级）。
    """
    # 角色等级表：数字越大权限越高。无 min_role 元数据的块视为公开（0 级即可见）。
    _ROLE_LEVEL = {"public": 0, "staff": 1, "manager": 2, "admin": 3}
    my_level = _ROLE_LEVEL.get((role or "public").lower(), 0)
    ...
    # 3) 阈值过滤 + 权限过滤：
    #    - 分数 ≥ threshold 才算相关；
    #    - 块的 min_role（默认 0）不得超过请求方角色等级，否则视为越权、丢弃。
    candidates = []
    for i, s in zip(ids[0], scores[0]):
        if s < threshold:
            continue
        blk = CHUNKS[i]
        if _ROLE_LEVEL.get(blk.get("min_role", "public"), 0) > my_level:
            continue  # 越权块：不召回（政企权限分级的关键控制点）
        candidates.append((blk, float(s)))
